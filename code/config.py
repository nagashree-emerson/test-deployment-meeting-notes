...
# CRITICAL: All config variables are loaded from Key Vault (if enabled) or .env file.
# No hardcoded values. Key Vault is primary, .env is fallback. Warnings for missing.
...
class Config:
    ...
    KEY_VAULT_SECRET_MAP = [
        ...
        # Encryption key (domain-specific)
        ("AGENT_ENCRYPTION_KEY", "agentops-secrets.agent_encryption_key"),
    ]

    # Model config for LLM kwargs
    ...
    def _load_keyvault_secrets(cls):
        ...
        try:
            ...
            logging.warning(f"Key Vault: failed to initialize client: {exc}")
            return {}
    ...
    def _validate_api_keys(cls):
        ...
        if provider == "openai" and not getattr(cls, "OPENAI_API_KEY", ""):
            raise ValueError("OPENAI_API_KEY not found for OpenAI provider")
        if provider == "azure" and not getattr(cls, "AZURE_OPENAI_API_KEY", ""):
            raise ValueError("AZURE_OPENAI_API_KEY not found for Azure provider")
        if provider == "anthropic" and not getattr(cls, "ANTHROPIC_API_KEY", ""):
            raise ValueError("ANTHROPIC_API_KEY not found for Anthropic provider")
        if provider == "google" and not getattr(cls, "GOOGLE_API_KEY", ""):
...
def _initialize_config():
    ...
    CONFIG_VARIABLES = [
        ...
        # Service Principal (conditional)
        "AZURE_TENANT_ID", "AZURE_CLIENT_ID", "AZURE_CLIENT_SECRET",
        # LLM / Model
        "MODEL_PROVIDER", "LLM_MODEL", "LLM_TEMPERATURE", "LLM_MAX_TOKENS", "AZURE_OPENAI_ENDPOINT",
        # API Keys
        "OPENAI_API_KEY", "AZURE_OPENAI_API_KEY", "ANTHROPIC_API_KEY", "GOOGLE_API_KEY",
        # Azure Content Safety
        "AZURE_CONTENT_SAFETY_ENDPOINT", "AZURE_CONTENT_SAFETY_KEY",
        # Azure AI Search (always .env)
        ...
        "LLM_MODELS",
        # Validation config path (for agent)
        "VALIDATION_CONFIG_PATH",
        # Version (for agent/observability)
        "VERSION",
    ...
    for var_name in CONFIG_VARIABLES:
        ...
        # LLM_MAX_TOKENS, OBS_AZURE_SQL_PORT: int
        ...
        elif value and var_name == "LLM_MODELS":
            try:
                import json
        ...
        if value is None or value == "":
            ...
            value = "" if var_name != "OBS_AZURE_SQL_TRUST_SERVER_CERTIFICATE" else "yes"
